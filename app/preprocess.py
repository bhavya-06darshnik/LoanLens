...
def explain_prediction(model, hf):
    # get SHAP values from H2O
    shap = model.predict_contributions(hf)

    # convert to Python-friendly dict
    df_shap = shap.as_data_frame()

    # The last column is the bias term; drop it
    df_shap = df_shap.iloc[:, :-1]

    # Extract the first row of SHAP values (single prediction)
    shap_dict = df_shap.iloc[0].to_dict()
